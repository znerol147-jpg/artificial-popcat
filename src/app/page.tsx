...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-dark/80 backdrop-blur-md border-b border-neon/20 py-4' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        <div className="text-2xl font-black tracking-tighter flex items-center gap-2">
          <span className="text-white">AI</span>
          <span className="text-neon drop-shadow-[0_0_10px_rgba(57,255,20,0.8)]">POPCAT</span>
        </div>
        
        {/* Desktop Nav */}
        <div className="hidden md:flex gap-8 items-center font-medium text-sm text-gray-300">
          {navLinks.map((link) => (
            <a key={link} href={`#${link.toLowerCase().replace(/ /g, '-')}`} className="hover:text-neon transition-colors duration-200">
              {link}
            </a>
          ))}
          <a href="#buy" className="bg-neon text-black px-5 py-2 rounded-full font-bold hover:bg-white hover:text-black hover:shadow-[0_0_20px_rgba(57,255,20,0.6)] transition-all duration-300">
            Buy $POPCAT
          </a>
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {navLinks.map((link) => (
              <a key={link} href={`#${link.toLowerCase().replace(/ /g, '-')}`} onClick={() => setIsOpen(false)} className="text-lg font-medium text-white hover:text-neon">
                {link}
              </a>
            ))}
          </motion.div>
      ...
      <motion.div style={{ y, opacity }} className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
        <div className="text-center lg:text-left z-10">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
            <motion.div variants={fadeInUp} className="inline-block border border-neon/50 bg-neon/10 text-neon px-4 py-1.5 rounded-full text-sm font-semibold mb-6 backdrop-blur-sm shadow-[0_0_15px_rgba(57,255,20,0.2)]">
            ...
            <motion.p variants={fadeInUp} className="text-lg text-gray-400 mb-8 max-w-xl mx-auto lg:mx-0">
              The internet's most iconic screaming cat, upgraded with AI. Neon green eyes. Zero fear. Maximum memes. It doesn't meow. It pumps.
            </motion.p>
            ...
            <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start mb-8">
              ...
              <a href="https://twitter.com/placeholder" target="_blank" rel="noreferrer" className="border border-white/20 bg-white/5 hover:bg-white/10 px-8 py-4 rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-2 backdrop-blur-sm">
            ...
            <motion.div variants={fadeInUp} className="flex items-center justify-center lg:justify-start gap-2 bg-dark-surface border border-white/10 p-2 rounded-lg max-w-md mx-auto lg:mx-0">
              <span className="text-gray-500 font-mono text-xs sm:text-sm truncate px-2">{contractAddress}</span>
        ...
        >
          ...
          >
            {/* Replace /popcat.jpg with your actual image path */}
            <Image src="/popcat.jpg" alt="Artificial Popcat" fill className="object-cover" priority />
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <motion.div variants={fadeInUp}>
              <h2 className="text-3xl md:text-5xl font-black mb-6">THE <span className="text-neon">UPGRADE</span></h2>
              <p className="text-gray-400 text-lg mb-6 leading-relaxed">
                ARTIFICIAL POPCAT was created in the depths of a neural network fed exclusively on green candles and Robinhood charts. The biological limitations of the original meme have been stripped away.
              </p>
              <p className="text-gray-400 text-lg leading-relaxed">
                We replaced the vocal cords with algorithmic trading APIs. We replaced the fur with liquid cooling. It doesn't scream for food anymore; it screams through timelines to raid X and bring meme energy to the blockchain.
              </p>
            </motion.div>
            <motion.div variants={fadeInUp} className="grid grid-cols-2 gap-4">
              {[
                { icon: <Activity className="text-neon mb-2" size={32}/>, title: "Hyper-Optimized", desc: "Built for speed and volatility." },
                { icon: <Shield className="text-neon mb-2" size={32}/>, title: "Unruggable AI", desc: "Contract renounced, LP burned." },
                { icon: <Zap className="text-neon mb-2" size={32}/>, title: "Zero Taxes", desc: "0/0 limits. Pure momentum." },
                { icon: <XLogo className="text-neon mb-2" size={32}/>, title: "X Raids", desc: "Programmed to dominate timelines." }
              ].map((feature, idx) => (
                <div key={idx} className="bg-black/50 border border-white/5 p-6 rounded-2xl hover:border-neon/50 transition-colors">
                  {feature.icon}
    ...
    <section id="tokenomics" className="py-24 relative">
      <div className="max-w-7xl mx-auto px-6">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="text-center mb-16">
          <h2 className="text-4xl md:text-6xl font-black mb-4">TOKEN<span className="text-neon">OMICS</span></h2>
          <p className="text-gray-400 max-w-2xl mx-auto">No complex math. Just simple, aggressive mechanics designed for the Robinhood launch meta.</p>
        </motion.div>
...
const Roadmap = () => {
  const phases = [
    { phase: "Phase 1: Boot Sequence", items: ["Website Launch & Socials", "Contract Deployment", "Initial X Raid Protocols", "DEX Listings"] },
    { phase: "Phase 2: Network Override", items: ["CoinGecko/CoinMarketCap Fast-Track", "Trending on Web3 Platforms", "Community AI Meme Generation", "10k+ Holders Target"] },
    { phase: "Phase 3: Robinhood Meta", items: ["Tier-1 CEX Pursuits", "Mainstream Media Infiltration", "Total Timeline Domination", "The Ultimate Green Candle"] }
  ];

  return (
    <section id="roadmap" className="py-24 relative">
      <div className="max-w-4xl mx-auto px-6">
        <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="text-4xl md:text-6xl font-black text-center mb-16">
...
const Footer = () => {
  ...
  <XLogo size={24} />
            ...
            <a href="https://t.me/ArtificialPopcatrh" className="bg-dark-surface border border-white/10 p-3 rounded-xl hover:bg-neon hover:text-black hover:border-neon transition-all">
              <Send size={24} />
            </a>
          </div>
        </div>
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-gray-600">
          ...
          <p className="max-w-xl text-center md:text-right">
            Disclaimer: $POPCAT is a meme coin with no intrinsic value or expectation of financial return. There is no formal team or roadmap. The coin is completely useless and for entertainment purposes only.
          </p>
        </div>
      </div>
    </footer>
